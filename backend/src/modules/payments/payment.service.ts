...
import { TelebirrGateway } from './gateways/telebirr.gateway';
import { PayPalGateway } from './gateways/paypal.gateway';
import { StripeGateway } from './gateways/stripe.gateway';
import { PaymentGateway, InitiatePaymentDTO, PaymentResult } from './gateways/paymentGateway.interface';
...
export class PaymentService {
  ...
  private initializeGateways() {
    this.gateways.set('TELEBIRR', new TelebirrGateway());
  ...
  private getGateway(gatewayName: string): PaymentGateway {
    ...
    if (!gateway) {
      throw ApiError.badRequest(`Payment gateway not supported: ${gatewayName}`);
    }
    return gateway;
  }
  ...
  async initiatePayment(data: {
    ...
    try {
      ...
      const idempotencyKey = data.idempotencyKey || PaymentSecurity.generateIdempotencyKey();
      ...
      await prisma.transactionAuditLog.create({
        data: PaymentSecurity.createAuditLog({
          userId: data.userId,
          bookingId: data.bookingId,
   ...
   */
  private async initiateBankTransfer(paymentId: string, data: any) {
    const bankDetails = {
      bankName: env.BANK_NAME || 'Dashen Bank',
      accountNumber: env.BANK_ACCOUNT_NUMBER || '1234567890',
      accountHolder: env.BANK_ACCOUNT_HOLDER || 'MAKI Platform',
      amount: data.amount,
      currency: data.currency,
  ...
  async processWebhook(data: {
    ...
    try {
      ...
      if (!payment) {
        throw ApiError.notFound('Payment not found for webhook');
      }
   ...
   */
  private async handlePaymentSuccess(paymentId: string, bookingId: string) {
    ...
    const { commission, netAmount } = PaymentSecurity.calculateCommission(
      payment.amount,
      env.PAYMENT_COMMISSION_PERCENTAGE
    );
  ...
  private async generateInvoice(paymentId: string) {
    try {
      ...
      const invoiceNumber = PaymentSecurity.generateInvoiceNumber();
   ...
   */
  async listUserPayments(userId: string, filters: any = {}) {
