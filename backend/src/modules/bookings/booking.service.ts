...
export class BookingService {
  ...
  static async createBooking(data: {
    ...
    if (existingBooking) {
      throw ApiError.conflict('You already have an active booking request for this property');
    }
    ...
    if (conflictingBooking) {
      throw ApiError.badRequest('Property is not available for selected dates');
    }
   ...
   */
  static async acceptBooking(bookingId: string, landlordId: string) {
    ...
    if (booking.landlordId !== landlordId) {
      throw ApiError.forbidden('Only landlord can accept this booking');
    }
  ...
  static async declineBooking(
    ...
    if (booking.landlordId !== landlordId) {
      throw ApiError.forbidden('Only landlord can decline this booking');
    }
  ...
  static async counterOffer(
    ...
    if (booking.landlordId !== landlordId) {
      throw ApiError.forbidden('Only landlord can make counter-offers');
    }

    if (booking.status !== 'REQUESTED' && booking.status !== 'COUNTER_OFFERED') {
      throw ApiError.badRequest(`Cannot counter-offer booking with status: ${booking.status}`);
    }
  ...
  static async cancelBooking(
    ...
    if (booking.status === 'PAID' || booking.status === 'ACTIVE') {
      const daysUntilMoveIn = Math.floor(
        (booking.moveInDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
      );

      const policy = booking.property.cancellationPolicy || 'MODERATE';

      if (policy === 'STRICT') {
        if (daysUntilMoveIn > 14) refundPercentage = 100;
        else if (daysUntilMoveIn > 7) refundPercentage = 50;
        else refundPercentage = 0;
      } else if (policy === 'MODERATE') {
        if (daysUntilMoveIn > 7) refundPercentage = 100;
        else if (daysUntilMoveIn > 3) refundPercentage = 50;
        else refundPercentage = 0;
      } else if (policy === 'FLEXIBLE') {
        if (daysUntilMoveIn > 3) refundPercentage = 100;
        else if (daysUntilMoveIn > 1) refundPercentage = 50;
        else refundPercentage = 0;
   ...
   */
  static async blockCalendarDates(
    propertyId: string,
    moveInDate: Date,
    durationMonths: number
  ) {
   ...
   * Release calendar dates
   */
  static async releaseCalendarDates(
    propertyId: string,
    moveInDate: Date,
    durationMonths: number
  ) {
   ...
   * Notify tenant
   */
  private static async notifyTenant(booking: any, eventType: string) {
   ...
   * Notify landlord
   */
  private static async notifyLandlord(booking: any, eventType: string) {
    const messages: Record<string, any> = {
      BOOKING_REQUEST: {
        title: 'New Booking Request',
        message: `${booking.tenant.firstName} has requested to book your property: ${booking.property.title}`,
